...
function startLawOfLargeNumbersSimulation() {
    ...
    drawTrajectory(currentIndex);
    document.getElementById('trajectoryInfoHM4').textContent = `Trajectory ${currentIndex + 1} of ${trajectories.length}`;
}

function toggleAutoScrollHM4() {
    if (!trajectories || trajectories.length === 0) {
        alert('No trajectories found. Please run the simulation first!');
        return;
    }
    const btn = document.getElementById('autoScrollBtnHM4');
    if (!autoscroll) {
        autoscroll = true;
